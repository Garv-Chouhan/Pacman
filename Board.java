...
public class Board extends JPanel implements ActionListener {
    ...
    private Color mazColor;
    private boolean inGame = false; // true for now
    ...
    private int[] dx, dy;
    private int[] ghosts_x, ghosts_y, ghosts_dx, ghosts_dy, ghostsSpeed;
    ...
    private void initVariable() {
        ...
        mazColor = new Color(5, 100, 5);
        d = new Dimension(400, 400);
        ghosts_x = new int[MAX_GHOSTS];
        ghosts_dx = new int[MAX_GHOSTS];
        ghosts_y = new int[MAX_GHOSTS];
        ghosts_dy = new int[MAX_GHOSTS];
        ghostsSpeed = new int[MAX_GHOSTS];
        dx = new int[4];
        dy = new int[4];
    ...
    private void doAnim() {
        ...
        if (pacAnimCount <= 0) {
            ...
            if (pacAnimPos == (PACMAN_ANIM_COUNT - 1) || pacAnimPos == 0) {
    ...
    private void drawScore(Graphics2D g2d) {
        ...
        for (i = 0; i < pacsLeft; i++) {
            g2d.drawImage(pacman3left, i * 28 + 8, SCREEN_SIZE + 1, this);
    ...
    private void moveGhost(Graphics2D g2d) {
        ...
        for (i = 0; i < N_GHOSTS; i++) {
            if (ghosts_x[i] % BLOCK_SIZE == 0 && ghosts_y[i] % BLOCK_SIZE == 0) {
                pos = ghosts_x[i] / BLOCK_SIZE + N_BLOCKS * (int) (ghosts_y[i] / BLOCK_SIZE);
                count = 0;

                if ((screenData[pos] & 1) == 0 && ghosts_dx[i] != 1) {
                    dx[count] = -1;
                    dy[count] = 0;
                    count++;
                }

                if ((screenData[pos] & 2) == 0 && ghosts_dy[i] != 1) {
                    dx[count] = 0;
                    dy[count] = -1;
                    count++;
                }

                if ((screenData[pos] & 4) == 0 && ghosts_dx[i] != -1) {
                    dx[count] = 1;
                    dy[count] = 0;
                    count++;
                }

                if ((screenData[pos] & 8) == 0 && ghosts_dy[i] != -1) {
                    dx[count] = 0;
                    dy[count] = 1;
                    count++;
                }
                ...
                if (count == 0) {
                    if ((screenData[pos] & 15) == 15) {
                        ...
                        ghosts_dx[i] = -ghosts_dx[i];
                        ghosts_dy[i] = -ghosts_dy[i];
                    ...
                    ghosts_dx[i] = dx[count];
                    ghosts_dy[i] = dy[count];

                }

            }

            ghosts_x[i] = ghosts_x[i] + (ghosts_dx[i] * ghostsSpeed[i]);
            ghosts_y[i] = ghosts_y[i] + (ghosts_dy[i] * ghostsSpeed[i]);
            drawGhost(g2d, ghosts_x[i] + 1, ghosts_y[i] + 1);

            if (pacman_x > (ghosts_x[i] - 12) && pacman_x < (ghosts_x[i] + 12)
                    && pacman_y > (ghosts_y[i] - 12) && pacman_y < ghosts_y[i] + 12
                    && inGame) {
                dying = true;
            }
        }
    ...
    private void drawPacman(Graphics g2d) {
        if (view_dx == -1) {
            drawPacmanLeft(g2d);
        } else if (view_dx == 1) {
            drawPacmanRight(g2d);
    ...
    private void drawPacmanUp(Graphics g2d) {
        switch (pacAnimPos) {
            case 1:
    ...
    private void drawPacmanDown(Graphics g2d) {
        switch (pacAnimPos) {
            case 1:
    ...
    private void drawPacmanLeft(Graphics g2d) {
        switch (pacAnimPos) {
            case 1:
    ...
    private void drawPacmanRight(Graphics g2d) {
    ...
    private void continueLevel() {
        ...
        for (i = 0; i < N_GHOSTS; i++) {
            ghosts_y[i] = 4 * BLOCK_SIZE;
            ghosts_x[i] = 4 * BLOCK_SIZE;
            ghosts_dy[i] = 0;
            ghosts_dx[i] = dx;
            dx = -dx;
            ...
            ghostsSpeed[i] = validSpeeds[random];
        }
